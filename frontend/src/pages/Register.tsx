    ...
    try {
      ...
      if (response.success) {
        if (response.requiresEmailConfirmation) {
          setStep('otp');
          setSuccess("Account created! Please check your email for the 8-digit code.");
        } else {
      ...
      <div style={{ width: "100%", maxWidth: "440px", backgroundColor: T.card, border: `1px solid ${T.border}`, borderRadius: "6px", padding: "2.25rem 2rem", boxShadow: "0 4px 28px rgba(46,18,8,0.09)" }}>
        ...
        {step === 'otp' && (
          <>
            ...
            <p style={{ fontFamily: T.sans, color: T.muted, fontSize: "0.85rem", marginTop: 0, marginBottom: "1.5rem" }}>
              We sent an 8-digit code to <strong style={{ color: T.brown }}>{email}</strong>. Check your spam folder if you don't see it.
            </p>
            ...
            <form onSubmit={handleVerifyOtp} style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              ...
              <div style={{ display: "flex", gap: "0.75rem", marginTop: "0.25rem" }}>
                <button type="button" onClick={() => { setStep('form'); setOtp(''); setError(''); setSuccess(''); setResendCooldown(0); }}
