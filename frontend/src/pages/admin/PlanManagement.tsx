...
import { Check, Edit, Plus, X, Loader2, Star, Info, Headphones, Video, Lock, ChevronUp, ChevronDown } from 'lucide-react';
...
const PlanManagement: React.FC = () => {
    ...
    const loadPlans = async () => {
        try {
            ...
            const transformedPlans = fetchedPlans.map(plan => ({
                ...
                // Ensure features have order
                features: (plan.features || []).map((feature, index) => ({
                    ...feature,
                    order: feature.order !== null && feature.order !== undefined ? feature.order : index + 1
        ...
        try {
            ...
            if (editingGroup.monthlyPlan) {
                const monthlyData = {
                    ...
                    // Ensure features have proper order
                    features: (editingGroup.monthlyPlan.features || []).map((feature, index) => ({
                        ...feature,
            ...
            if (editingGroup.yearlyPlan) {
                const yearlyData = {
                    ...
                    // Ensure features have proper order
                    features: (editingGroup.yearlyPlan.features || []).map((feature, index) => ({
                        ...feature,
                        order: feature.order || index + 1
                    }))
                };
            ...
            setPlans(prev => {
                ...
                savedPlans.forEach(savedPlan => {
                    const transformedPlan = {
                        ...
                        // Ensure features have order
                        features: (savedPlan.features || []).map((feature, index) => ({
                            ...feature,
                            order: feature.order !== null && feature.order !== undefined ? feature.order : index + 1
                        })).sort((a, b) => (a.order || 0) - (b.order || 0))
                    };
    ...
    const getSortedFeatures = (): PlanFeature[] => {
        if (!editingGroup) return [];
        
        const baseFeatures = editingGroup.monthlyPlan?.features || editingGroup.yearlyPlan?.features || [];
    ...
    const updateFeature = (index: number, value: string) => {
        ...
        if (sortedFeatures[index]) {
            const featureId = sortedFeatures[index].id;
    ...
    const addFeature = () => {
        ...
        const basePlanId = updatedGroup.monthlyPlan?.id || updatedGroup.yearlyPlan?.id;
        const sortedFeatures = getSortedFeatures();
        const maxOrder = sortedFeatures.length > 0 
            ? Math.max(...sortedFeatures.map(f => f.order || 0)) 
            : 0;
    ...
    const removeFeature = (index: number) => {
        ...
        if (sortedFeatures[index]) {
            ...
            if (updatedGroup.monthlyPlan) {
                const newFeatures = (updatedGroup.monthlyPlan.features || []).filter(f => f.id !== featureId);
                // Reorder remaining features
                const reorderedFeatures = newFeatures.map((feature, idx) => ({
                    ...feature,
                    order: idx + 1
                }));
                updatedGroup.monthlyPlan = { ...updatedGroup.monthlyPlan, features: reorderedFeatures };
            }
            
            if (updatedGroup.yearlyPlan) {
                const newFeatures = (updatedGroup.yearlyPlan.features || []).filter(f => f.id !== featureId);
                // Reorder remaining features
                const reorderedFeatures = newFeatures.map((feature, idx) => ({
                    ...feature,
                    order: idx + 1
                }));
                updatedGroup.yearlyPlan = { ...updatedGroup.yearlyPlan, features: reorderedFeatures };
            }
        }
    ...
    const moveFeatureUp = (index: number) => {
        if (!editingGroup || index === 0) return;

        const updatedGroup = { ...editingGroup };
        const sortedFeatures = getSortedFeatures();
        
        if (sortedFeatures[index] && sortedFeatures[index - 1]) {
            const featureId = sortedFeatures[index].id;
            const prevFeatureId = sortedFeatures[index - 1].id;

            if (updatedGroup.monthlyPlan) {
                const newFeatures = [...(updatedGroup.monthlyPlan.features || [])];
                const featureIndex = newFeatures.findIndex(f => f.id === featureId);
                const prevFeatureIndex = newFeatures.findIndex(f => f.id === prevFeatureId);
                
                if (featureIndex >= 0 && prevFeatureIndex >= 0) {
                    // Swap orders
                    const tempOrder = newFeatures[featureIndex].order;
                    newFeatures[featureIndex] = {
                        ...newFeatures[featureIndex],
                        order: newFeatures[prevFeatureIndex].order
                    };
                    newFeatures[prevFeatureIndex] = {
                        ...newFeatures[prevFeatureIndex],
                        order: tempOrder
                    };
                    updatedGroup.monthlyPlan = { ...updatedGroup.monthlyPlan, features: newFeatures };
    ...
    const moveFeatureDown = (index: number) => {
        ...
        const sortedFeatures = getSortedFeatures();
        if (index === sortedFeatures.length - 1) return;

        const updatedGroup = { ...editingGroup };
        
        if (sortedFeatures[index] && sortedFeatures[index + 1]) {
            const featureId = sortedFeatures[index].id;
            const nextFeatureId = sortedFeatures[index + 1].id;

            if (updatedGroup.monthlyPlan) {
                const newFeatures = [...(updatedGroup.monthlyPlan.features || [])];
                const featureIndex = newFeatures.findIndex(f => f.id === featureId);
                const nextFeatureIndex = newFeatures.findIndex(f => f.id === nextFeatureId);
                
                if (featureIndex >= 0 && nextFeatureIndex >= 0) {
                    // Swap orders
                    const tempOrder = newFeatures[featureIndex].order;
                    newFeatures[featureIndex] = {
                        ...newFeatures[featureIndex],
                        order: newFeatures[nextFeatureIndex].order
                    };
    ...
    const sortedFeatures = editingGroup ? getSortedFeatures() : [];
            ...
            {groupedPlans.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {groupedPlans.map((group) => {
                        ...
                        const displayFeatures = [...(basePlan.features || [])]
                            .sort((a, b) => {
                                const aOrder = a.order || 0;
                                const bOrder = b.order || 0;
                                return aOrder - bOrder;
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-2 gap-2 text-sm">
                                                ...
                                                <div className="p-2 bg-gray-50 rounded">
                                                    ...
                                                    <div className="text-gray-600">{basePlan.card_limit || 0}/month</div>
                                                </div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <div className="flex items-center justify-between rounded-lg border p-4">
                                                <div className="space-y-0.5">
                                                    ...
                                                    <div className="text-sm text-gray-500">Total chat sessions</div>
                                                </div>
                                                <Input
                                                    id="chat-limit"
                                            ...
                                            <div className="flex items-center justify-between rounded-lg border p-4">
                                                <div className="space-y-0.5">
                                                    ...
                                                    <div className="text-sm text-gray-500">Monthly personalized cards</div>
                                                </div>
                                                <Input
                                                    id="card-limit"
                                                    type="number"
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-center">
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-500">Features shown on pricing page (drag to reorder)</p>
                                            </div>
                                        ...
                                        <div className="space-y-3">
                                            {sortedFeatures.length === 0 ? (
                                                ...
                                                sortedFeatures.map((feature, index) => (
                                                    <div key={feature.id || index} className="flex items-center gap-2 border rounded-lg p-3 bg-white hover:bg-gray-50">
                                        ...
                                        <Alert>
                                            <Info className="h-4 w-4" />
                                            <AlertDescription className="text-sm">
                                                <div className="font-medium mb-1">Feature Ordering:</div>
                                                <ul className="list-disc pl-4 space-y-1">
                                                    <li>Use up/down arrows to reorder features</li>
                                                    <li>Features will be displayed in this order on the pricing page</li>
                                                    <li>Order numbers start from 1</li>
                                                </ul>
                                            </AlertDescription>
                                        </Alert>
                                    </div>
